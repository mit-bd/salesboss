...
export default function OrderDetailPage() {
  ...
  const childOrders = mockOrders.filter((o) => o.parentOrderId === order.id);
  const parentOrder = order.parentOrderId ? mockOrders.find((o) => o.id === order.parentOrderId) : null;
  ...
  const timeline: TimelineEvent[] = [
    ...
    ...(order.isRepeat && parentOrder
      ? [{
          ...
          title: "Repeat Order",
          description: `Linked to parent order #${parentOrder.id}`,
          date: order.createdAt,
    ...
    ...(childOrders.length > 0
      ? [{
          ...
          title: "Upsell / Repeat",
          description: `${childOrders.length} repeat order(s) created`,
          date: childOrders[0].createdAt,
            ...
            <div className="rounded-xl border border-border bg-card p-5 card-shadow">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 pt-4 border-t border-border">
                <div>
                  ...
                  <p className="text-sm font-medium text-foreground flex items-center gap-1"><Package className="h-3.5 w-3.5" /> {order.productTitle}</p>
                </div>
                <div>
                ...
                <div>
                  ...
                  <p className="text-sm font-medium text-foreground">{order.orderSource}</p>
                </div>
                <div>
                  ...
                  <div key={step} className="flex-1 flex flex-col items-center gap-1.5">
                    <div
                      className={cn(
                        ...
                        step < order.followupStep ? "bg-success text-success-foreground" :
                        step === order.followupStep ? STEP_COLORS[step - 1] :
              ...
              {timeline.map((event, i) => (
                <div key={i} className="flex gap-3">
                  <div className="flex flex-col items-center">
                    <div className={cn("flex h-8 w-8 shrink-0 items-center justify-center rounded-lg", event.color)}>
                      {event.icon}
                    </div>
              ...
              <div className="mt-4 pt-4 border-t border-border">
                ...
                {childOrders.map((child) => (
                  <div
                    ...
                    className="flex items-center gap-2 p-2 rounded-lg hover:bg-muted/50 cursor-pointer transition-fast"
                  >
